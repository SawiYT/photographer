	...
	return (
		<div
			...
			<ul
				...
				{items.map((item, idx) => (
					<li
						className='relative w-[350px] max-w-full rounded-2xl border border-b-0 flex-shrink-0  px-8 py-6 md:w-[450px] overflow-hidden'
						style={{
							backgroundImage: 'url(/bg2.jpg)', // Ensure the path is correct relative to the public directory
						...
						<blockquote>
							...
							<div className='relative z-20 mt-6 flex flex-row items-center'>
								<span className='flex flex-col gap-1'>
									<span className=' text-sm leading-[1.6] text-gray-200 font-normal'>{item.name}</span>
									<span className=' text-sm leading-[1.6] text-gray-200 font-normal'>{item.title}</span>
								</span>
							</div>
						</blockquote>
					</li>
